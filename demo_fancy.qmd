...
The Medicare beneficiary dataset for this analysis pulled down just a subset of the rows and columns of the full data - `r scales::label_comma()(nrow(df))` rows and `r ncol(df)` columns. The analysis is focusing in on only the total beneficiary counts (`tot_benes`) in `r focus_state` counties in `r params$start_year` to `r params$end_year`.
...
We can visualize all `r {focus_state}`'s county growth rate data for comparison in a map (@fig-growth-rate).
